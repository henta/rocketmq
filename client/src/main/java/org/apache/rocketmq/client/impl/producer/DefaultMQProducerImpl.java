...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                ...
                //选一个messageQueue,采用轮询的策略拿queue
                MessageQueue mqSelected = this.selectOneMessageQueue(topicPublishInfo, lastBrokerName);
                if (mqSelected != null) {
                    mq = mqSelected;
                    //将选择的队列所在的brokername记录
                    ...
                    try {
                        ...
                        //这个异常可能导致消息重复发送，例如，发送消息超时(RemotingTimeoutException)，实际Broker接收到该消息并处理成功。因此，Consumer在消费时，需要保证幂等性。
                        endTimestamp = System.currentTimeMillis();
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        //先尝试从ConcurrentMap中拿topic信息
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            //没有topic信息的话会从nameservice更新topic信息
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            //如果还没有topicPublishInfo可用且Broker支持自动创建Topic，则会先创建一个新的topic
